    ...
    <div>
      ...
      <div className="bg-gradient-to-r from-blue-700 to-blue-500 text-white rounded-lg p-5 mb-6 flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <button onClick={handleReset} className="bg-white/15 hover:bg-white/25 text-sm px-3 py-1.5 rounded transition-colors flex items-center gap-1" title={t('shell_reset_confirm')}>
      ...
      <section>
        ...
        <div className="grid grid-cols-[repeat(auto-fill,minmax(190px,1fr))] gap-3">
          <CountTile title={lang==='vi'?'Quản lý bút toán nhà cung cấp':'Manage Supplier Line Items'} value="" icon="ti-file-invoice" onClick={() => navigate('/list/invoices')} />
          <CountTile title={lang==='vi'?'Phân tích thanh toán NCC':'Supplier Payment Analysis'} sub={lang==='vi'?'Thanh toán đang mở':'Open Payments'} value={0} icon="ti-cash" onClick={() => navigate('/app/payment-blocks')} />
          <KpiTile title={lang==='vi'?'Phân tích xử lý hóa đơn':'Invoice Processing Analysis'} subtitle={lang==='vi'?'Hôm nay':'Today'} kpiKey="overduePayables" />
          <KpiTile title={lang==='vi'?'Tổng quan công nợ phải trả':'Accounts Payable Overview'} kpiKey="cashDiscountUtilization" />
        </div>
      </section>
    </div>
  );
