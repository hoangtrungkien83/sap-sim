...
export default function FioriShell({ children }) {
  ...
  const [dark, setDark] = useState(() => { try { return localStorage.getItem("fiori-dark") === "1"; } catch { return false; } });
  useEffect(() => { document.documentElement.classList.toggle("fiori-dark", dark); try { localStorage.setItem("fiori-dark", dark ? "1" : "0"); } catch {} }, [dark]);
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[var(--fiori-page-bg)]">
      ...
      <header className="bg-[var(--fiori-shell-bg)] text-white h-12 flex items-center px-3 gap-2 sm:gap-4 shrink-0">
        ...
        <LangSwitcher />
        <button onClick={() => setDark(v => !v)} className="flex items-center gap-1 px-2 py-1 rounded text-xs text-white hover:bg-white/10 border border-white/20" title={dark ? "Light mode" : "Dark mode"} aria-label="Toggle dark mode"><i className={`ti ${dark ? "ti-sun" : "ti-moon"} text-base`} aria-hidden="true" /><span className="hidden sm:inline">{dark ? "Light" : "Dark"}</span></button>
        <button
