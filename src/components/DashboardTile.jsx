...
function NewsTile({ tile }) {
  const title = typeof tile.title === 'string' ? tile.title : tile.title?.vi ?? '';
  return (
    <>
      <div
        ...
        aria-hidden="true"
      >SAP NEWS</div>
      <p className="text-sm font-medium text-[var(--fiori-text-primary)] leading-snug mt-2 flex-1">{title}</p>
      <p className="text-xs text-[var(--fiori-text-secondary)] mt-auto flex items-center gap-1">
        <i className="ti ti-calendar text-[10px]" aria-hidden="true" />{tile.dt}
      </p>
    </>
  );
}
...
function ProfileTile({ tile, lang }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full gap-2 py-2">
      <div className="w-12 h-12 rounded-full bg-[var(--fiori-link)] text-white flex items-center justify-center text-base font-semibold">{initials}</div>
...
export default function DashboardTile({ tile }) {
  ...
  const inner = {
    kpi:     <KpiTile     tile={tile} lang={lang} />,
    chart:   <ChartTile   tile={tile} lang={lang} />,
    list:    <ListTile    tile={tile} lang={lang} onNav={navigate} />,
    news:    <NewsTile    tile={tile} />,
    profile: <ProfileTile tile={tile} lang={lang} />,
    status:  <StatusTile  tile={tile} lang={lang} />,
  }[tile.type] ?? null;

  return (
    <div
      className="bg-white border border-[var(--fiori-tile-border)] rounded-lg p-3 flex flex-col min-h-[130px] hover:shadow-md hover:border-[var(--fiori-link)] transition-all"
      role="article"
      aria-label={typeof tile.title === 'string' ? tile.title : tile.title?.[lang] ?? tile.name ?? ''}
    >
      {inner}
    </div>
  );
